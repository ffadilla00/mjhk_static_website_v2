...
function clearPreview(wrap,img){$(wrap).classList.add("hidden");$(img).removeAttribute("src")}
function showPreview(file,wrap,img){if(!file){clearPreview(wrap,img);return}const reader=new FileReader();reader.onload=()=>{$(img).src=reader.result;$(wrap).classList.remove("hidden")};reader.readAsDataURL(file)}
...
function openF(n){$("#"+n+"FormWrap").classList.remove("hidden");$("#"+n+"Form").reset();$("#"+n+"Id").value="";if(n==="kajian"){$("#posterLama").value="";clearPreview("#posterPreviewWrap","#posterPreview")}if(n==="keuangan"){$("#keuanganImageLama").value="";clearPreview("#keuanganPreviewWrap","#keuanganPreview");updatePeriodPreview()}}
...
$("#posterFile").onchange=()=>{const file=$("#posterFile").files[0];if(validateFile(file))showPreview(file,"#posterPreviewWrap","#posterPreview");else $("#posterFile").value=""};
$("#keuanganImage").onchange=()=>{const file=$("#keuanganImage").files[0];if(validateFile(file))showPreview(file,"#keuanganPreviewWrap","#keuanganPreview");else $("#keuanganImage").value=""};
...
window.editK=id=>{const x=kajianData.find(x=>x.id===id);openF("kajian");$("#kajianId").value=x.id;$("#jenis").value=x.jenis||"";$("#judul").value=x.judul||"";$("#tema").value=x.tema||"";$("#penceramah").value=x.penceramah||"";$("#tanggal").value=x.tanggal||"";$("#waktu").value=x.waktu||"";$("#kajianStatus").value=x.status||"publish";$("#posterLama").value=x.poster_url||"";if(x.poster_url){$("#posterPreview").src=x.poster_url;$("#posterPreviewWrap").classList.remove("hidden")}};

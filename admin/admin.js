...
function clearPreview(wrap,img,meta,note){$(wrap).classList.add("hidden");$(img).removeAttribute("src");$(meta).textContent="";$(note).classList.add("hidden")}
function showPreview(file,wrap,img,meta,note){if(!file){clearPreview(wrap,img,meta,note);return}const reader=new FileReader();reader.onload=()=>{$(img).src=reader.result;$(meta).textContent=`${file.name} - ${(file.size/(1024*1024)).toFixed(2)} MB`;$(note).classList.add("hidden");$(wrap).classList.remove("hidden")};reader.readAsDataURL(file)}
function showStoredPreview(url,wrap,img,meta,note){if(!url){clearPreview(wrap,img,meta,note);return}$(img).src=url;$(meta).textContent="Gambar saat ini tersimpan di Supabase Storage.";$(note).classList.remove("hidden");$(wrap).classList.remove("hidden")}
...
function openF(n){$("#"+n+"FormWrap").classList.remove("hidden");$("#"+n+"Form").reset();$("#"+n+"Id").value="";if(n==="kajian"){$("#posterLama").value="";clearPreview("#posterPreviewWrap","#posterPreview","#posterPreviewMeta","#posterStorageNote")}if(n==="keuangan"){$("#keuanganImageLama").value="";clearPreview("#keuanganPreviewWrap","#keuanganPreview","#keuanganPreviewMeta","#keuanganStorageNote");updatePeriodPreview()}}
...
async function rollbackFile(bucket,url){try{await removeFile(bucket,url)}catch(err){toast(`Rollback file gagal: ${err.message}`,"error")}}
function yid(u){const m=(u||"").match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([A-Za-z0-9_-]{6,})/);return m?m[1]:""}

$("#posterFile").onchange=()=>{const file=$("#posterFile").files[0];if(validateFile(file))showPreview(file,"#posterPreviewWrap","#posterPreview","#posterPreviewMeta","#posterStorageNote");else $("#posterFile").value=""};
$("#keuanganImage").onchange=()=>{const file=$("#keuanganImage").files[0];if(validateFile(file))showPreview(file,"#keuanganPreviewWrap","#keuanganPreview","#keuanganPreviewMeta","#keuanganStorageNote");else $("#keuanganImage").value=""};
$("#kajianForm").onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;let newPoster=null,databaseSaved=false;setBusy(form,true);try{
  const id=+$("#kajianId").value||null,file=$("#posterFile").files[0],oldPoster=$("#posterLama").value||null;let poster=oldPoster;if(!validateFile(file))return;
  if(file){newPoster=await upload("poster-kajian",file,"kajian");poster=newPoster}
  ...
  const {error}=id?await db.from("kajian").update(payload).eq("id",id):await db.from("kajian").insert(payload);if(error)throw error;databaseSaved=true;if(newPoster&&oldPoster)await removeFile("poster-kajian",oldPoster);closeF("kajian");toast(id?"Agenda berhasil diperbarui.":"Agenda berhasil ditambahkan.","success");await loadAll();
}catch(err){if(newPoster&&!databaseSaved)await rollbackFile("poster-kajian",newPoster);toast(err.message,"error")}finally{setBusy(form,false)}};
...
$("#keuanganForm").onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;let newImage=null,databaseSaved=false;setBusy(form,true);try{
  const id=+$("#keuanganId").value||null,a=$("#periodeAwal").value,b=$("#periodeAkhir").value,file=$("#keuanganImage").files[0],oldImage=$("#keuanganImageLama").value||"";if(b<a)throw new Error("Periode akhir tidak boleh lebih awal dari periode awal.");if(!id&&!file)throw new Error("File laporan wajib diupload.");if(!validateFile(file))return;let image=oldImage;
  if(file){newImage=await upload("laporan-keuangan",file,"laporan");image=newImage}
  const payload={periode_awal:a,periode_akhir:b,tanggal_publish:$("#publishDate").value,image_url:image,status:$("#keuanganStatus").value};const {error}=id?await db.from("keuangan").update(payload).eq("id",id):await db.from("keuangan").insert(payload);if(error)throw error;databaseSaved=true;if(newImage&&oldImage)await removeFile("laporan-keuangan",oldImage);closeF("keuangan");toast(id?"Laporan berhasil diperbarui.":"Laporan berhasil ditambahkan.","success");await loadAll();
}catch(err){if(newImage&&!databaseSaved)await rollbackFile("laporan-keuangan",newImage);toast(err.message,"error")}finally{setBusy(form,false)}};
window.editK=id=>{const x=kajianData.find(x=>x.id===id);openF("kajian");$("#kajianId").value=x.id;$("#jenis").value=x.jenis||"";$("#judul").value=x.judul||"";$("#tema").value=x.tema||"";$("#penceramah").value=x.penceramah||"";$("#tanggal").value=x.tanggal||"";$("#waktu").value=x.waktu||"";$("#kajianStatus").value=x.status||"publish";$("#posterLama").value=x.poster_url||"";showStoredPreview(x.poster_url,"#posterPreviewWrap","#posterPreview","#posterPreviewMeta","#posterStorageNote")};
